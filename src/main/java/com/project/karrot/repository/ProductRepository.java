...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    List<Product> findByLocationAndCategory(@Param("location") Location location, @Param("category") Category category); // 지역별 및 카테고리별 상품 조회

    @Query(" select p from Product p where p.member = :member")
    List<Product> findByMember(@Param("member") Member member); // 회원 등록 상품 전체 조회

    @Query(" select p from Product p where p.member = :member and p.productStatus = :status")
    List<Product> findByMemberAndStatus(@Param("member") Member member, @Param("status") ProductStatus status); // 회원별 진행단계별 조회
